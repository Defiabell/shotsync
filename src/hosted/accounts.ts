...
function configured(env: HostedEnv): boolean {
  try { return !!env.EMAIL && !!env.EMAIL_FROM && new URL(env.PUBLIC_ORIGIN).protocol === 'https:' && !!env.TURNSTILE_SECRET_KEY; } catch { return false; }
}
...
async function mailToken(env: HostedEnv, user: UserRow, kind: 'verify' | 'reset'): Promise<void> {
  const token = randomToken();
  const hash = await tokenHash(token);
  await env.DB.prepare('INSERT INTO account_tokens(hash,user_id,kind,expires_at,auth_version) VALUES(?,?,?,?,?)')
    .bind(hash, user.id, kind, Date.now() + (kind === 'verify' ? DAY : 30 * 60_000), user.auth_version).run();
  try { await sendAccountMail(env, user.email, kind, token); }
  catch (error) {
    await env.DB.prepare('DELETE FROM account_tokens WHERE hash=?').bind(hash).run();
    throw error;
  }
}
...
export async function createPendingUser(db: D1Database, id: string, email: string, passwordHash: string, verifiedLimit: number): Promise<void> {
  await db.prepare(`INSERT INTO users(id,email,password_hash,created_at) SELECT ?,?,?,?
    WHERE (SELECT COUNT(*) FROM users WHERE verified_at IS NOT NULL)<?
      AND (SELECT COUNT(*) FROM users WHERE verified_at IS NULL)<200
    ON CONFLICT(email) DO NOTHING`).bind(id, email, passwordHash, Date.now(), verifiedLimit).run();
}
export async function handleAccounts(request: Request, env: HostedEnv): Promise<Response | null> {
  ...
  if (route === 'devices' || route.startsWith('devices/')) {
    ...
    if (!user.verified) return fail('Verify your email first', 403);
  ...
  if (!['register', 'login', 'resend-verification', 'forgot-password', 'verify', 'reset-password'].includes(route)) return fail('Not found', 404);
  const ip = await ipKey(request);
  if (await limited(env, `ip:${ip}`, 30, 600)) return fail('Try again later', 429);
  const body = await readJson(request);
  if (route === 'verify' || route === 'reset-password') {
    if (typeof body.token !== 'string' || !/^[a-f0-9]{64}$/.test(body.token) || !validPassword(body.password)) return fail('A valid link and password of 10–128 characters are required');
    const hash = await tokenHash(body.token), now = Date.now(), kind = route === 'verify' ? 'verify' : 'reset';
    const token = await env.DB.prepare('SELECT t.user_id FROM account_tokens t JOIN users u ON u.id=t.user_id WHERE t.hash=? AND t.kind=? AND t.expires_at>? AND t.auth_version=u.auth_version').bind(hash, kind, now).first();
    if (!token) return fail('Link expired or already used');
    if (await limited(env, `password:${ip}`, 10, 600)) return fail('Try again later', 429);
    ...
    const update = route === 'verify'
      ? `UPDATE users SET password_hash=?,verified_at=?,auth_version=auth_version+1 WHERE verified_at IS NULL AND (SELECT COUNT(*) FROM users WHERE verified_at IS NOT NULL)<? AND id=(SELECT user_id FROM account_tokens WHERE hash=? AND kind='verify' AND expires_at>? AND auth_version=users.auth_version) RETURNING id`
      : `UPDATE users SET password_hash=?,auth_version=auth_version+1 WHERE verified_at IS NOT NULL AND id=(SELECT user_id FROM account_tokens WHERE hash=? AND kind='reset' AND expires_at>? AND auth_version=users.auth_version) RETURNING id`;
    const result = await env.DB.prepare(update).bind(...(route === 'verify' ? [password, now, cap, hash, now] : [password, hash, now])).first();
    if (!result) return fail(route === 'verify' ? 'Trial is full or link already used' : 'Link expired or already used', 409);
    return reply({ ok: true }, 200, cookie('', 0));
  }
  ...
  if (!configured(env)) return fail('Registration and email are temporarily unavailable', 503);
  if (!(await challenge(request, env, body.turnstileToken))) return fail('Please complete the security check', 403);
  if (await limited(env, `mail:${emailKey}`, 3, 3600) || await limited(env, `mail-ip:${ip}`, 10, 3600)) return fail('Try again later', 429);
  let mailReserved = false;
  ...
  if (route === 'register') {
    ...
    if (!user) {
      // Bound account creation even when email delivery is exhausted or unavailable.
      if (await limited(env, 'pending-registrations', 200, 86400) || await limited(env, 'mail-global', 200, 86400)) return fail('Try again later', 429);
      mailReserved = true;
      const cap = Math.max(1, Math.min(100, Number.parseInt(env.REGISTRATION_LIMIT || '100', 10) || 100));
      if (await limited(env, 'password-global', 120, 60)) return fail('Authentication is busy. Try again shortly.', 429);
      const password = await withPasswordWork(env.DB, () => hashPassword(body.password as string));
      await createPendingUser(env.DB, crypto.randomUUID(), email, password, cap);
      user = await env.DB.prepare('SELECT * FROM users WHERE email=?').bind(email).first<UserRow>();
      if (!user) return fail('Trial is full. Please try again later.', 409);
    }
  }
  if (user && ((route === 'forgot-password' && user.verified_at !== null) || (route !== 'forgot-password' && user.verified_at === null))) {
    if (!mailReserved && await limited(env, 'mail-global', 200, 86400)) return fail('Try again later', 429);
    try { await mailToken(env, user, route === 'forgot-password' ? 'reset' : 'verify'); }
    catch { return fail('Email delivery unavailable. Please try again later.', 503); }
  }
  return reply({ ok: true });
}
...
export async function cleanupAccounts(db: D1Database): Promise<void> {
  ...
  await db.batch([
    ...
    db.prepare('DELETE FROM users WHERE id IN (SELECT id FROM users WHERE verified_at IS NULL AND created_at<? AND NOT EXISTS (SELECT 1 FROM account_tokens WHERE user_id=users.id AND expires_at>?) LIMIT 100)').bind(now - 7 * DAY, now),
  ]);
}
